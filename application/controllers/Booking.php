...
class Booking extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($dtb < 1) {
            ...
            $data['temp'] = $this->db->query("select * from temp1 where id_user='$id_user'")->result_array();
        }
    ...
    public function tambahBooking()
    {
        ...
        $d = $this->db->query("Select*from buku1 where id='$id_buku'")->row();
        ...
        $temp = $this->ModelBooking->getDataWhere('temp1', ['id_buku' => $id_buku])->num_rows();

        $userid = $this->session->userdata('id_user');

        //cek jika sudah memasukan 3 buku untuk dibooking dalam keranjang
        $tempuser = $this->db->query("select*from temp1 where id_user ='$userid'")->num_rows();

        //cek jika masih ada booking buku yang belum diambil
        $databooking = $this->db->query("select*from booking1 where id_user='$userid'")->num_rows();
        if ($databooking > 0) {
        ...
        $this->ModelBooking->insertData('temp1', $isi);
    ...
    {
        $email_user = $this->session->userdata('email');
        $data['detail_booking'] = $this->ModelBooking->getDataWhere('temp1', ['email_user' => $email_user])->result();
    ...
    public function hapusbooking()
    {
        ...
        $kosong = $this->db->query("select*from temp1 where id_user='$id_user'")->num_rows();
    ...
    public function bookingSelesai($where)
    {
        ...
        $this->db->query("UPDATE buku1, temp1 SET buku1.dibooking=buku1.dibooking+1, buku1.stok=buku1.stok-1 WHERE buku1.id=temp1.id_buku");

        $tglsekarang = date('Y-m-d');
        $isibooking = [
            'id_booking' => $this->ModelBooking->kodeOtomatis('booking1', 'id_booking'),
        ...
        $this->ModelBooking->insertData('booking1', $isibooking);
    ...
    public function info()
    {
        ...
        $data['items'] = $this->db->query("select*from booking1 bo, booking_detail1 d, buku1 bu where d.id_booking=bo.id_booking and d.id_buku=bu.id and bo.id_user='$where'")->result_array();
    ...
    public function exportToExcel()
    {
        ...
        $data['useraktif'] = $this->ModelUser->cekData(['id' => $this->session->userdata('id_user')])->result();
        $data['items'] = $this->db->query("select*from booking1 bo, booking_detail1 d, buku1 bu where d.id_booking=bo.id_booking and d.id_buku=bu.id and bo.id_user='$id_user'")->result_array();
